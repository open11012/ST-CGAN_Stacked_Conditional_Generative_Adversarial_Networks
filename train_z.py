from utils.data_loder_Z import make_datapath_list, ImageDataset, ImageTransform
from models.ST_CGAN_Z import Generator, Discriminator
...
def get_parser():
    ...
    parser.add_argument('-b', '--batch_size', type=int, default=133, help='Batch size')
...
def train_model(G1, G2, D1, D2, dataloader, val_dataset, num_epochs, parser, save_model_name='model'):
    ...
    for epoch in range(num_epochs+1):
        ...
        for imagesG1, imagesG2, gt_shadow, gt in tqdm(dataloader):

            # if size of minibatch is 1, an error would be occured.
            if imagesG2.size()[0] == 1:
                continue

            imagesG1 = imagesG1.to(device)
            imagesG2 = imagesG2.to(device)
            gt = gt.to(device)
            gt_shadow = gt_shadow.to(device)

            mini_batch_size = imagesG1.size()[0]
            ...
            # for D1
            detected_shadow = G1(imagesG1)
            fake1 = torch.cat([imagesG1, detected_shadow], dim=1)
            real1 = torch.cat([imagesG1, gt_shadow], dim=1)
            out_D1_fake = D1(fake1.detach())
            out_D1_real = D1(real1)# .detach() is not required as real1 doesn't have grad

            # for D2
            fakeG2 = torch.cat([imagesG2, detected_shadow], dim=1)
            realG2 = torch.cat([imagesG2, gt_shadow], dim=1)
            shadow_removal_image = G2(fakeG2)
            fake2 = torch.cat([fakeG2, shadow_removal_image], dim=1)
            real2 = torch.cat([realG2, gt], dim=1)
...
def main(parser):
    G1 = Generator(input_channels=4, output_channels=1)
    G2 = Generator(input_channels=4, output_channels=3)
    D1 = Discriminator(input_channels=5)
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=16, pin_memory=True) #num_workers=4
