'''
此代码是在test 代码上修改而来的，主要修改将代码指定cpu 运行，测试自己的图片
此代码用于测试无人机图像和卫星图像
'''
...
def get_parser():
    ...
    parser.add_argument('-i', '--image_path', type=str, default='./dataset/test_uav/7.jpg', help='file path of image you want to test')
    parser.add_argument('-o', '--out_path', type=str, default='./test_result', help='saving path')  # 在这里更改输入的图像
...
def test_own_image(G1, G2, path, out_path, size, img_transform):
    ...
    with torch.no_grad():
        ...
        save_image(grid, out_path + './rs/grid_9400_' + path.split('/')[-1])

        detected_shadow = transforms.ToPILImage(mode='L')(unnormalize(detected_shadow)[0, :, :, :])
        detected_shadow = detected_shadow.resize((width, height), Image.LANCZOS)
        detected_shadow.save(out_path + '/rs/detected_shadow_9400_' + path.split('/')[-1])

        shadow_removal_image = transforms.ToPILImage(mode='RGB')(unnormalize(shadow_removal_image)[0, :, :, :])
        shadow_removal_image = shadow_removal_image.resize((width, height), Image.LANCZOS)
        shadow_removal_image.save(out_path + '/rs/shadow_removal_image_9400_' + path.split('/')[-1])

        # plt.imshow(grid)  # 显示图片
        # # plt.show()
        plt.imshow(detected_shadow)  # 显示图片
        plt.show()
        plt.imshow(shadow_removal_image)  # 显示图片
        plt.show()
...
def main(parser):
    ...
    '''load'''
    G1_weights = torch.load('./model/ST-CGAN_G1_3640.pth',map_location='cpu') #指定cpu运行
    G1.load_state_dict(fix_model_state_dict(G1_weights))

    G2_weights = torch.load('./model/ST-CGAN_G2_3640.pth',map_location='cpu')
